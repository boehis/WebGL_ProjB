...
function main() {
	...
	g_u_ViewMatrix = gl.getUniformLocation(gl.program, 'u_ViewMatrix');
	g_u_ProjMatrix = gl.getUniformLocation(gl.program, 'u_ProjMatrix');
	if (!g_u_ModelMatrix || !g_u_ViewMatrix || !g_u_ProjMatrix) {
	...
	g_viewMatrix.setLookAt(5, 5, 3, -1, -2, -0.5, 0, 0, 1);
	g_projMatrix.setPerspective(42.0, 1.0, 1.0, 1000.0);
	// Pass the model, view, and projection matrix to the uniform variable respectively
	gl.uniformMatrix4fv(g_u_ModelMatrix, false, g_modelMatrix.elements);
	gl.uniformMatrix4fv(g_u_ViewMatrix, false, g_viewMatrix.elements);
	gl.uniformMatrix4fv(g_u_ProjMatrix, false, g_projMatrix.elements);
...
function drawShapes() {
	...
	g_modelMatrix.translate(-0.4, -0.4, 0.0);  // 'set' means DISCARD old matrix,
	// (drawing axes centered in CVV), and then make new
	// drawing axes moved to the lower-left corner of CVV. 
	g_modelMatrix.scale(0.2, 0.2, 0.2);
	// if you DON'T scale, cyl goes outside the CVV; clipped!
	g_modelMatrix.rotate(g_currentAngle, 0, 1, 0);  // spin around y axis.
	// Drawing:
	...
	g_modelMatrix.scale(0.3, 0.3, 0.3);
	// Make it smaller:
	g_modelMatrix.rotate(g_currentAngle, 0, 1, 1);  // Spin on YZ axis
	// Drawing:		
	// Pass our current matrix to the vertex shaders:
	gl.uniformMatrix4fv(g_u_ModelMatrix, false, g_modelMatrix.elements);
